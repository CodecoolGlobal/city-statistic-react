...
export default function Comments(props) {
  ...
  function handleUpvote(e) {
    let element = document.getElementById(
      e.currentTarget.closest("div").dataset.id
    );
    ...
    axios
      .put("http://localhost:8080/rate/" + id, { rate: "upvote" })
      .then((res) => console.log(res));
  }

  function handleDownvote(e) {
    let element = document.getElementById(
      e.currentTarget.closest("div").dataset.id + "down"
    );
    ...
    element.innerHTML = value;
    axios
      .put("http://localhost:8080/rate/" + id, { rate: "downvote" })
      .then((res) => console.log(res));
  }
  ...
  function handleReplySubmit(e) {
    ...
    if (!replyContent == "") {
      axios
        .post("http://localhost:8080/reply/" + id, { reply: replyContent })
        .then((res) => console.log(res));
    }
    e.target.reset();
            ...
            <li>
              <div className="user-comment-card">
                ...
                <div className="user-info">
                  ...
                  <div className="user__comment">{comment.comment}</div>
                  <div className="user__comment__reaction">
                    <div onClick={handleUpvote} data-id={comment.id}>
                      <Icon icon={thumbsUp} color="green" width="24" />
                      <small id={comment.id}>{comment.upvote}</small>
                    </div>
                    <div
                      ...
                      class="down"
                    >
                      <Icon icon={thumbsDown} color="red" width="24" />{" "}
                      <small id={comment.id + "down"}>{comment.downvote}</small>
                    </div>
                  </div>
                  <button id="replySubmit" onClick={handleReply}>
                    Reply
                  </button>
                  <form
                  ...
                  >
                    <textarea
                      ...
                      placeholder="Share your opinion about this comment..."
                    />
